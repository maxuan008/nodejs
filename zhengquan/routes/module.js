...
jsmodule.GetUsermodlist_V2= function GetUsermodlist_V2(prjid,userid,callback){
    
	mongodb.open(function(err,db){
		if(err) {return callback(err);}
	    var Module=require('./module');
		var n=0, t=0, reu='', nulltmp=null;
	    var cursor =db.collection('usermodule').find({'projid':prjid, 'userid':userid,'cancelstatus':1 });
	    
	   // console.log('prjid:' + prjid + ',userid:'+ userid);
	    
	    cursor.toArray(function(err, doc) {
	    	//console.log(doc);
	    	if(doc.length==0)   return  callback(err,nulltmp);
	    	else
	    		{reu='';
	    		async.eachSeries(doc, function(value, callback){
	    			 
	    			 ModuleFindOne(value.projid, value.modid , function(err,modoc) {
	    				 //console.log(modoc);
	    				 var hidmodIDtmp='hidmod_id'+ n ,  hidmodName='hidmod_name'+ n ;
	    				if(modoc != null) {
	    					var hidmodtmp='hidmod_id'+ modoc.modid;  //console.log(hidmodtmp);
		    				 reu=reu +  "<li><input type=\'hidden\' id=\'hidmodtmp" + modoc.modid+"\'  value=\'"+modoc.modid+"\' ><a onclick='showconent("+modoc.modid+")' href=\'/"+modoc.jsname+"zhengquan\'>" + modoc.name + "</a></li>";
	    			   ...
	    			   if(err)  console.log(err);  
	    			  // console.log(reu);
	    			  return 	callback(err,reu);});    //async.eachSeries   end
...
function ModuleFindOne(prjid,modid,callback) {
	mongodb.open(function(err,db){
		if(err) {return callback(err);}
		//console.log(modid);
		
		db.collection('module').findOne({'projid':prjid, 'modid':modid ,'cancelstatus':1 },function(err, doc) {
